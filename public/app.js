...
document.getElementById('loadMoreBtn').addEventListener('click',function(){shown+=8;renderProducts();});

function filterByCategory(cat){
  filter=cat; shown=8;
  document.querySelectorAll('.fbtn').forEach(function(b){b.classList.toggle('active',b.dataset.filter===cat);});
  renderProducts();
  setTimeout(function(){document.getElementById('products').scrollIntoView({behavior:'smooth'});},80);
}
...
function changeQty(id,d){
  const it=cart.find(function(x){return x.id===id;}); if(!it)return;
  it.qty+=d;
  if(it.qty<=0) cart=cart.filter(function(x){return x.id!==id;});
  saveCart(); updateCart();
}
function updateCart(){
  const cnt=cart.reduce(function(s,i){return s+i.qty;},0);
  document.getElementById('cartBadge').textContent=cnt;
  document.getElementById('bnavBadge').textContent=cnt;
  const tot=cart.reduce(function(s,i){return s+i.price*i.qty;},0);
...
function renderCheckoutForm(){
  ...
  body.innerHTML =
    ...
    + '<div class="checkout-fg"><input type="text" class="finp" id="co-name" placeholder="Nom complet *" autocomplete="name"></div>'
    + '<div class="checkout-row2">'
    + '<div class="checkout-fg"><input type="email" class="finp" id="co-email" placeholder="E-mail *" autocomplete="email" inputmode="email"></div>'
    + '<div class="checkout-fg"><input type="tel" class="finp" id="co-phone" placeholder="Téléphone" autocomplete="tel" inputmode="tel"></div>'
    + '</div>'
    + '</div>'
    + '<div class="checkout-section">'
    + '<div class="checkout-section-title">📍 Adresse de livraison</div>'
    + '<div class="checkout-fg"><input type="text" class="finp" id="co-address" placeholder="Adresse *" autocomplete="street-address"></div>'
    + '<div class="checkout-row3">'
    + '<div class="checkout-fg"><input type="text" class="finp" id="co-city" placeholder="Ville *" autocomplete="address-level2"></div>'
    + '<div class="checkout-fg"><input type="text" class="finp" id="co-zip" placeholder="Code postal *" autocomplete="postal-code" inputmode="numeric"></div>'
    + '<div class="checkout-fg"><input type="text" class="finp" id="co-country" placeholder="Pays" value="France" autocomplete="country-name"></div>'
    + '</div>'
    + '</div>'
...
async function submitOrder(){
  ...
  const result = await api('/orders', {
    method: 'POST',
    body: JSON.stringify({
      customer: { name: name, email: email, phone: phone, address: address, city: city, zip: zip, country: country },
      items: cart.map(function(it){ return { id: it.id, qty: it.qty }; })
    })
  });

  if(result.ok){
    showOrderSuccess(result.data.orderNumber, result.data.total);
    cart = [];
    saveCart();
    updateCart();
  } else {
...
function toggleWish(e,id){
  ...
  localStorage.setItem('ff_wish',JSON.stringify(Array.from(wishlist)));
  renderProducts();
}
document.getElementById('wishBnav').addEventListener('click',function(){
  if(!wishlist.size){toast('Aucun favori pour l\'instant ♡');return;}
  document.getElementById('products').scrollIntoView({behavior:'smooth'});
  toast(wishlist.size+' favori'+(wishlist.size>1?'s':'')+' ♥');
});

document.getElementById('burgerBtn').addEventListener('click',function(){document.getElementById('mobMenu').classList.add('open');});
document.getElementById('mobX').addEventListener('click',closeMob);
document.getElementById('mobOv').addEventListener('click',closeMob);
...
function secretTap(){
  ...
  sTimer=setTimeout(function(){sTaps=0;},2500);
}
if(window.location.hash==='#admin'){
  window.addEventListener('load',function(){setTimeout(openLogin,400);});
}
...
function openLogin(){
  ...
  setTimeout(function(){document.getElementById('loginInp').focus();},350);
}
...
async function checkLogin(){
  ...
  if(result.ok){
    ...
    document.getElementById('loginErr').textContent='🔒 Trop d\'essais — bloqué '+result.data.remainingMinutes+' min';
  } else {
    document.getElementById('loginInp').value='';
    document.getElementById('loginErr').style.display='block';
    const rem = result.data.remainingAttempts || 0;
...
async function checkAdminStatus(){
  ...
  isAdmin = result.ok && result.data.isAdmin;
  return isAdmin;
}
...
document.querySelectorAll('.atab').forEach(function(tab){
  tab.addEventListener('click',async function(){
    ...
    document.querySelectorAll('.atc').forEach(function(c){c.classList.remove('active');});
...
async function saveProduct(){
  ...
  const payload = { name: name, cat: cat, price: price, oldPrice: oldS?parseFloat(oldS):null, badge: badge||null, img: img, link: link, emoji: emoji, rating: rating };
...
function resetForm(){
  ['f-name','f-cat','f-price','f-oldprice','f-badge','f-img','f-link'].forEach(function(id){document.getElementById(id).value='';});
...
async function renderAdminList(){
  ...
  if(!result.ok){ toast('Erreur de chargement'); return; }
  ...
  list.innerHTML=allProducts.map(function(p){
...
async function renderOrdersList(){
  ...
  if(!result.ok){ toast('Erreur de chargement des commandes'); return; }
  const orders = result.data.orders;
  document.getElementById('orderCount').textContent = orders.length;

  const pendingCount = orders.filter(function(o){ return ['pending','paid','sourcing'].indexOf(o.status) !== -1; }).length;
  const totalAmount = orders.reduce(function(s,o){ return s + o.total_amount; }, 0);
  ...
  list.innerHTML = orders.map(function(o){
    const statusOptions = Object.keys(STATUS_LABELS).map(function(val){
      return '<option value="'+val+'" '+(o.status===val?'selected':'')+'>'+STATUS_LABELS[val]+'</option>';
    }).join('');
    ...
    return '<div class="order-card">'
      ...
      + '</div>'
      + '<select class="order-status-sel" onchange="updateOrderStatus('+o.id+', this.value)">'+statusOptions+'</select>'
      + '</div>'
...
async function updateOrderStatus(orderId, status){
  const result = await api('/admin/orders/'+orderId+'/status', { method:'PUT', body: JSON.stringify({ status: status }) });
  if(result.ok){ toast('Statut mis à jour ✓'); }
  else { toast('Erreur de mise à jour'); }
}

async function toggleItemSourced(itemId){
  const result = await api('/admin/order-items/'+itemId+'/sourced', { method:'PUT' });
  if(!result.ok) toast('Erreur');
}
...
async function changePwd(){
  ...
  const result = await api('/admin/change-password', { method:'POST', body: JSON.stringify({ newPassword: p1 }) });
  if(result.ok){
...
function toast(msg){
  ...
  tTimer=setTimeout(function(){document.getElementById('toast').classList.remove('show');},3000);
}
